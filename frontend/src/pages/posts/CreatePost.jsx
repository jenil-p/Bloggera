    ...
    <CreatePostModal isOpen={isOpen} onClose={onClose} title="Create New Post">
      <div className="p-4 h-full rounded-lg shadow-lg">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {categories.map(category => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm ${categoryIds.includes(category._id) ? 'bg-blue-500 text-white' : 'background text-theme hover:bg-gray-300'}`}
              >
                {category.name}
              </button>
            ))}
        ...
        <div className="flex z-50 justify-end space-x-2 mt-4">
          ...
          <button onClick={handleSubmit} className="relative inline-flex items-center justify-start px-6 py-3 overflow-hidden font-medium transition-all bg-blue-500 rounded-xl group">
            ...
            <span className="relative w-full text-left text-white transition-colors duration-200 ease-in-out group-hover:text-white">{loading ? 'Posting...' : 'Post'}</span>
          </button>
        </div>
      </div>
    </CreatePostModal>
